...
namespace DotNetCleanTemplate.Gateway.Features.Transformers;
...
internal sealed partial class AddAntiforgeryTokenResponseTransform(IAntiforgery antiforgery, ILogger<AddAntiforgeryTokenResponseTransform> logger) : ResponseTransform
{
    ...
    /// </summary>
    /// <param name="context">The <see cref="ResponseTransformContext"/> containing the current HTTP context and proxy response.</param>
    /// <returns>A completed <see cref="ValueTask"/> when processing is finished.</returns>
    public override ValueTask ApplyAsync(ResponseTransformContext context)
    {
        if (!context.HttpContext.Request.RouteValues.ContainsKey("catch-all") || context.HttpContext.Response.ContentType?.Contains("text/html", StringComparison.Ordinal) != true)
        {
    ...
    static partial void LogXsrfTokenAddedToResponseForRequestPathRequestpath(ILogger<AddAntiforgeryTokenResponseTransform> logger, string requestPath);
}